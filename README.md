...
**Статус:** v0.6.0 — MVP, live-verified на реальном аккаунте. ~430 тестов, билд чистый.
...
|---|---|---|
| **Просмотры профилей** | Заходит на профили рекрутёров из поиска, с dwell-паузой (как человек прочитал) | People |
| **Smart Connect** | Добавляет в сеть релевантных людей (без note, per-page, со стопом на лимит) | Relationships |
| **Engagement** | Лайкает посты из ленты + ~⅓ из них комментирует clarifying-вопросом | Insights |
...
- **Human-in-the-loop только где необратимо.** Публикация поста — только после твоего «Одобрить». Лайки/коннекты/просмотры — автономны в безопасном темпе.
...
```bash
npm install
npm test               # vitest run (~430 тестов)
...
**v0.6.0** — Profile Views (live-verified), Smart Connect (per-page, reliable STOP), Engagement (likes + clarifying comments), Content pipeline v2 (ideas → approve → авто-публикация), live Dash-виджет, SSI-движок.
