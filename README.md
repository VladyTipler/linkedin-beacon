...
2. Открой сайдбар Beacon с **любой** страницы → экран **SSI** покажет реальный балл; кнопка обновления тянет свежие данные.

**Как это работает (источник данных):**
- **Primary — внутренний API.** Service worker дёргает `/sales-api/salesApiSsi` (тот же эндпоинт, что гидрирует страницу `/sales/ssi`) и получает чистый JSON с баллом, 4 столпами и рангами. Куки сессии (`li_at` и пр., HttpOnly) браузер прикрепляет сам через `credentials: 'include'` + `host_permissions`; вручную читаем только `JSESSIONID` (не-HttpOnly) для заголовка `csrf-token` (permission `cookies`). Работает **с любой страницы, без открытой вкладки LinkedIn, без визуального мелькания**. Раз в сутки + по кнопке.
- **Fallback — DOM-парсер.** Если открыта сама `/sales/ssi`, content script парсит страницу напрямую (`DomSelectorStrategy` → `TextScanStrategy`) — мгновенно и как резерв, если LinkedIn поменяет/закроет API.

> Оба источника возвращают один доменный `SsiSnapshot` (DIP/OCP) — ядро не знает, откуда пришли данные.
