...
**Статус:** v0.11.1 — MVP, live-verified на реальном аккаунте. 574 теста, билд чистый.
...
**v0.11.1** — **PYMK-добор при частичном поиске:** когда лимит поиска LinkedIn отдаёт лишь 1-2 коннектных, прогон больше не встаёт на них — в ТОМ ЖЕ пассе добирает остаток дневного бюджета из PYMK (раньше фолбэк включался только при нулевом поиске → приходилось запускать второй раз). Дневной кап соблюдён.
